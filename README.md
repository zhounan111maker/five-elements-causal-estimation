...
> | 配置 | ATE 偏差 | 估计量标准差 | 结论 |
> |------|---------|-------------|------|
> | 含泄漏特征（α=1） | −74%（27/30 种子胜出） | 40.59 → 10.15（**塌缩 4.0×**） | 看起来更好，实为泄漏指纹 |
> | 清洁特征（α=0，无 Y(0) 信息） | +2.3%（15/30 胜出，配对 t 检验 **p = 0.892**） | 正常 | 五行特征无显著增益 |
>
> **另发现真值定义错误**：ATE 真值应为 τ = 0.15 × E[Y(0)]（全样本 = 660.61），原代码误用对照组均值（637.27），本身即带 −3.53% 偏差。
>
> **结论**：89.8% 是泄漏伪影，不是方法学贡献。泄漏的真正指纹是**估计量方差塌缩**（本例 4.0×），而不是偏差下降。
>
> ---
>
> **➡️ 完整诊断、可复现代码与修正后的负结果论文（新仓库）：**
> **https://github.com/zhounan111maker/dml-target-leakage-diagnosis**
>
> **📄 详细更正说明：见 [CORRECTION.md](CORRECTION.md)**
>
> 本仓库已归档（archived）并转为只读，作为透明更正记录保留。**⚠️ 下方正文中所有 89.8% / 92.3% / 54.8% 等数字均已失效，不可引用。**
...
>
> ⛔ **上述"89.8%"已撤回**：该数字源于目标泄漏伪影，详见本页顶部更正声明与新仓库 [dml-target-leakage-diagnosis](https://github.com/zhounan111maker/dml-target-leakage-diagnosis)。
...
> [!CAUTION]
> **本仓库报告的核心结论「五行特征使 DML 的 ATE 估计偏差降低 89.8%」经受控复查后不成立。**
>
> **根因：100% 目标泄漏（target leakage）**
> 派生特征 `wood_trans_efficiency = Total_Trans_Amt / Credit_Limit` 直接由基线结果变量 Y(0)（`Total_Trans_Amt`）构造。即使删除原始列，泄漏仍通过派生特征进入 DML 协变量。也就是说，模型在"估计"时已经偷看了答案。
>
> **受控消融诊断（30 个随机种子，仅切换泄漏通道 α）**
>
> | 配置 | ATE 偏差 | 估计量标准差 | 结论 |
> |------|---------|-------------|------|
> | 含泄漏特征（α=1） | −74%（27/30 胜） | 40.59 → 10.15（塌缩 4.0×） | 看起来更好，实为泄漏指纹 |
> | 清洁特征（α=0，无 Y(0) 信息） | +2.3%（15/30 胜，配对 t 检验 p=0.892） | 正常 | 五行特征无显著增益 |
>
> **另发现真值定义错误**：ATE 真值应为 τ = 0.15 × E[Y(0)]（全样本，= 660.61），原代码误用对照组均值（637.27），引入 −3.53% 的基线偏差。
>
> **结论**：89.8% 是泄漏伪影，不是方法学贡献。泄漏的典型指纹是**估计量方差塌缩**（本例 4.0×）而非偏差下降。
>
> **完整诊断、可复现代码与修正后的负结果论文**：见仓库 [dml-target-leakage-diagnosis](https://github.com/zhounan111maker/dml-target-leakage-diagnosis)。
>
> 本仓库保留作为透明更正记录，现已进入只读维护状态。
...
> ⛔ **本节所有数字受目标泄漏污染，已撤回，不可引用。** 保留原文仅为透明留档。
...
> ⛔ **本节的"五维验证"未包含最关键的一项：泄漏审计。** 所有 ✅ 均建立在含泄漏特征的配置上，不能作为方法有效性的证据。
...
> ⛔ **下表两行数字均已撤回**：所谓"跨域泛化"只是因为同一泄漏通道在另一份数据上重复生效。
...
> ⛔ **该引用条目对应的结论已撤回，请勿引用。** 如确需引用，请引用新仓库的负结果诊断研究：
> Zhou Nan (2026). *A Controlled Diagnostic of Target Leakage in DML Covariates*. GitHub.
> https://github.com/zhounan111maker/dml-target-leakage-diagnosis
...
**⚠️ 本仓库已归档（read-only）。核心结论因目标泄漏已撤回。**

**请移步 ➡️ [dml-target-leakage-diagnosis](https://github.com/zhounan111maker/dml-target-leakage-diagnosis)**
